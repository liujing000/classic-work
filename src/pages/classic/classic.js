...
export default class classic extends Component {
  loadCategories() {
    this.props.dispatch({
      type: 'classic/loadCategoriesAll',
  ...
  render() {
    const { classic: { categoriesAll }, loading } = this.props;
    const { selectedRowKeys,status,page,entry,index } = this.state;
    const { getFieldDecorator } = this.props.form; 
  
    //table的数据加key
    let newList = [];
    let list = categoriesAll;
    if(list) {
    ...
    return (
      <PageHeaderWrapper title="经典dddd">
